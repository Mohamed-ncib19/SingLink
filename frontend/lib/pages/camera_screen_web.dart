...
class CameraScreenWeb extends StatefulWidget {
  const CameraScreenWeb({Key? key}) : super(key: key);

  @override
  _CameraScreenWebState createState() => _CameraScreenWebState();
}
...
class _CameraScreenWebState extends State<CameraScreenWeb> {
  ...
  Future<bool> _requestRequiredPermissions() async {
    ...
    if (denied && mounted) {
      setState(() {
        _permissionMessage =
            'SignLink needs camera access for signs and microphone access while recording. Enable both permissions in browser or system settings.';
      });
    }
  ...
  Future<void> _captureAndPredict() async {
    ...
    try {
      ...
      final hasForeground = _frameContainsForeground(imageBytes);

      final response = await _sendToBackend(imageBytes);
  ...
  void _speakTranslation() async {
    if (translation.isEmpty || _flutterTts == null) return;
